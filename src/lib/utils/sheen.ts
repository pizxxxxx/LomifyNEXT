 ...
 *
 * Зачем скрипт для чисто визуального эффекта. Блик обязан работать в обе стороны: на вход
 * полоса едет вправо, на уход — возвращается на исходную позицию вместе с плиткой. Само
 * движение делает `transition` в app.css (переход, в отличие от `@keyframes`, умеет
 * разворачиваться с середины пути). От скрипта нужно только одно: понять, где границы
 * карточки, и держать на ней класс ровно пока курсор внутри.
 *
 ...
 * и переход курсора с обложки на подпись выглядел бы как уход с карточки — блик поехал бы
 * назад, хотя курсор ещё на плитке. Поэтому от найденного корня поднимаемся вверх, пока
